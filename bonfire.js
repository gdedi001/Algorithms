...
function updateInventory(curInv, newInv) {
    ...
    for (var i = 0; i < newInv.length; i++){
        for (var x = 0; x < curInv.length; x++){
            // check if the new item exists in current inv
            if (newInv[i][itemName] === curInv[x][itemName]) {
                console.log(curInv[x] + " | " + newInv[i])
                // update our current inventory with new incoming amount
                curInv[x][amount] += newInv[i][amount];
                break;
            }
            // if unable to match items push this item onto updated
            if (x === curInv.length-1) {
                updated.push(newInv[i]);
                break;
            }
        }

        // if the store is empty, push all new items into inventory
        if (curInv.length === 0) {
            updated.push(newInv[i]);
        }
    }
